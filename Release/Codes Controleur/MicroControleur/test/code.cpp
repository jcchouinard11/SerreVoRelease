...
/*     lv_init();
 ...
 /*    digitalWrite(42, HIGH);
    ...
    delay(5000);
    spiSD.begin(48, 41, 45, 40);
    //spiSD.setDataMode(SPI_MODE0);
    if (!SD.begin(40,spiSD)) {
    ...
    if (!SD.exists(filePath)&&cardType) {
...
/* 
    ...
    // ------------------------------------------------
    // Initialisation du BLE (section commentée dans le code original)
    // ------------------------------------------------
    ...
    /* xTaskCreatePinnedToCore(Task_LVGL, "Task_LVGL", 4096*12, NULL, 1, NULL, 0);
    xTaskCreatePinnedToCore(Task_Tick, "Task_LVGL", 4096*12, NULL, 1, NULL, 1);
    xTaskCreatePinnedToCore(Task_Screen_Update, "Task_Screen_Update", 4096, NULL, 2, NULL, 0);
    xTaskCreatePinnedToCore(Task_BLE, "Task_BLE", 4096*10, NULL, 4, NULL, 1);
    xTaskCreatePinnedToCore(detectSensors, "Task_Sensor", 4096, NULL, 3, NULL, 0); */
